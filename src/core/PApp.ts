...
export default class PApp extends EventEmitter {
  ...
  constructor({ baseScreenWidth, baseScreenHeight, renderTargetID }: { baseScreenWidth: number; baseScreenHeight: number; renderTargetID: string }) {
    super();
  ...
  set CurrentLevel(newLevel: PLevel | null) {
    ...
    if (newLevel) this.addToStage(newLevel.Container);
  ...
  public addToStage(target: PIXI.Container): void {
    ...
    if (target instanceof SSprite || target instanceof CContainer) target.onAddedToStage(this);
  ...
  public removeFromStage(target: PIXI.Container): void {
    ...
    if (target instanceof SSprite || target instanceof CContainer) target.onRemovedFromStage(this);
