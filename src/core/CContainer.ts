...
export default class CContainer extends PIXI.Container {
  ...
  public onAddedToStage(GameApp: PApp) {
    ...
    this.children.map((children) => {
      if (children instanceof CContainer || children instanceof SSprite) children.onAddedToStage(GameApp);
    });
  }
  ...
  private notifyOnStageForNewElements() {
    this.addListener('childAdded', (child) => {
      if (this.bIsOnStage && this.GameApp) if (child instanceof CContainer || child instanceof SSprite) child.onAddedToStage(this.GameApp);
    });
  }
  ...
  public onRemovedFromStage(GameApp: PApp) {
    ...
    this.children.map((children) => {
      if (children instanceof CContainer || children instanceof SSprite) children.onRemovedFromStage(GameApp);
    });
  }
  ...
  protected onResize(): void {
    ...
    if (this.bShouldScaleWithBaseRes) this.reScaleWithBaseRes();
  }
